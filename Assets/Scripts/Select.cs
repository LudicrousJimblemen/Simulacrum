		...
		if (Input.GetMouseButtonDown(0)) {
			if (Physics.Raycast(OrthoRay(), out UnitHit, Mathf.Infinity, ~(1 << 8))) {
				...
				foreach (GameObject p in persons) {
					...
					if (p.GetComponent<Unit>().Selected && p != UnitHit.collider.gameObject && !Input.GetKey(KeyCode.LeftShift))
						p.GetComponent<Unit>().Selected = false;
				}
			} else {
				foreach (GameObject p in persons) {
					if (p == null) {
					...
					if (p.GetComponent<Unit>().Selected && !Input.GetKey(KeyCode.LeftShift))
						p.GetComponent<Unit>().Selected = false;
				}
				inMarquee = true;
